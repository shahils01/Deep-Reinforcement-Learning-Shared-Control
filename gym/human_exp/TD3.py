...
class td3(object):
    def __init__(self, a_dim, s_dim, a_bound, GAMMA = 0.99,TAU = 0.0005, MEMORY_CAPACITY = int(5e5),BATCH_SIZE = 128,LR_A = 0.0001,LR_C = 0.001):
        ...
        q1 = self._build_c1(self.S1,self.S2, self.a,)
        q2 = self._build_c2(self.S1,self.S2, self.a,)
        self.a_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='Actor')
        self.c1_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='Critic1')
        self.c2_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='Critic2')

        ema = tf.train.ExponentialMovingAverage(decay=1 - TAU)          # soft replacement
        #self.global_step = tf.Variable(0, trainable=False)
        # LR_A = tf.compat.v1.train.exponential_decay(0.0001,self.global_step,300000, 1, staircase=True)
        # LR_C = tf.compat.v1.train.exponential_decay(0.001,self.global_step,300000, 1, staircase=True)

        def ema_getter(getter, name, *args, **kwargs):
            return ema.average(getter(name, *args, **kwargs))

        target_update = [ema.apply(self.a_params), ema.apply(self.c1_params),ema.apply(self.c2_params)]      # soft update operation

        _,a2 = self._build_a(self.S1_,self.S2_, reuse=True, custom_getter=ema_getter)   # replaced target parameters
        epsilon = tf.random_normal(tf.shape(a2), stddev=0.7)
        #epsilon = tf.clip_by_value(epsilon, -1, 1)
        a2 = a2 + epsilon
        a_ = tf.nn.softmax(a2)
        #target_noise = tf.random_normal(tf.shape(a_), stddev=0.2)
        #target_noise = tf.clip_by_value(target_noise, -0.5, 0.5)
        #a_n = tf.nn.softmax(a_)#+target_noise)
        q1_ = self._build_c1(self.S1_,self.S2_, a_, reuse=True, custom_getter=ema_getter)
        q2_ = self._build_c2(self.S1_,self.S2_, a_, reuse=True, custom_getter=ema_getter)
        ...
        self.td_error = tf.losses.mean_squared_error(labels=q_target, predictions=q1)+tf.losses.mean_squared_error(labels=q_target, predictions=q2)
        self.ctrain = tf.train.AdamOptimizer(LR_C).minimize(self.td_error, var_list=[self.c1_params,self.c2_params])#,global_step=self.global_step)
    ...
    def loader(self):
        ...
        loader.restore(self.sess,tf.train.latest_checkpoint('TD3_final_net'))
