...
class TurtleBot2Env(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self, ros_ws_abspath):
        ...
        * /kobuki/laser/scan: Laser Readings
        ...
        rospy.Subscriber("/burger37/odom", Odometry, self._odom_callback)
        rospy.Subscriber("/cmd_human",Twist,self._joy_callback) #############################joy##########
        rospy.Subscriber("/burger37/scan", LaserScan, self._laser_scan_callback)

        self._cmd_vel_pub = rospy.Publisher('/burger37/cmd_vel', Twist, queue_size=1)
    ...
    def _check_all_sensors_ready(self):
        ...
        #self._check_joy_ready()
        self._check_laser_scan_ready()
        rospy.logdebug("ALL SENSORS READY")

    def _check_joy_ready(self):
        self.joy = None
        rospy.logdebug("Waiting for /wamv/odom to be READY...")
        while self.joy is None and not rospy.is_shutdown():
            try:
                self.joy = rospy.wait_for_message("/cmd_human", Twist, timeout=1.0)
                rospy.logdebug("Current /cmd_human READY=>")

            except:
                rospy.logerr("Current /cmd_human not ready yet, retrying for getting joy")
        return self.joy
    ...
    def _check_odom_ready(self):
        ...
        while self.odom is None and not rospy.is_shutdown():
            try:
                self.odom = rospy.wait_for_message("/burger37/odom", Odometry, timeout=5.0)
    ...
    def _check_laser_scan_ready(self):
        ...
        rospy.logdebug("Waiting for /burger37/scanto be READY...")
        while self.laser_scan is None and not rospy.is_shutdown():
            try:
                self.laser_scan = rospy.wait_for_message("/burger37/scan", LaserScan, timeout=5.0)
                rospy.logdebug("Current /kobuki/laser/scan READY=>")

            except:
                rospy.logerr("Current /burger37/scan not ready yet, retrying for getting laser_scan")
    ...
    def move_base(self, linear_speed, angular_speed, epsilon=0.05, update_rate=10, min_laser_distance=-1):
        ...
        #rospy.sleep(0.2)  #use for other speed
        time.sleep(0.2) #use for normal time
        #time.sleep(0.04)
        """
