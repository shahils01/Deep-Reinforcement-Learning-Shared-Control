...
class TurtleBot2GoalEnv(turtlebot2_env.TurtleBot2Env):
    def __init__(self):
        """
        ...
        LoadYamlFileParamsTest(rospackage_name="openai_ros",
                               ...
                               yaml_file_name="turtlebot2_goal.yaml")

        # Here we will add any init functions prior to starting the MyRobotEnv
        super(TurtleBot2GoalEnv, self).__init__(ros_ws_abspath)
        ...
        high = numpy.array([7,7,3.14,14,14])
        low = numpy.array([-1*7,-1*7,-1*3.14,-1*14,-1*14])
    ...
    def _init_env_variables(self):
        ...
        self.desired_point.x = random.uniform(1,5)
        self.desired_point.y = random.uniform(-2,2)
    ...
    def _get_obs(self):
        ...
        observations = [round(x_position, 2),round(y_position, 2),round(base_yaw, 2),round(xdiff, 2),round(ydiff, 2)]
    ...
    def _compute_reward(self, observations, done):
        ...
        xdiff = observations[3]
        ydiff = observations[4]
        ...
        if not done:
            ...
            if distance_difference < 0.0:
                ...
                reward = abs(100 * (numpy.pi - d_theta) * distance_difference)
            else:
                rospy.logerr("ENCREASE IN DISTANCE BAD")
                reward = -abs(200 * d_theta * distance_difference)
