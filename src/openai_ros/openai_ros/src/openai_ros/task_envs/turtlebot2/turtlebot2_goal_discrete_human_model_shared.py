...
class TurtleBot2DiscreteHumanModelEnv(turtlebot2_joy_env.TurtleBot2Env):
    def __init__(self):
        ...
        high = numpy.hstack((numpy.array([3.14,0.5,1,0.5,1]),6*numpy.ones([self.new_ranges])))
        ...
        low = numpy.hstack((numpy.array([-3.14,-0.5,-1,-0.5,-1]),numpy.zeros([self.new_ranges])))
    ...
    def _set_action(self, action):
        ...
        if action == 0:
            ...
            if self.last_action != 2 or 3:
                odometry = self.get_odom()
                self.prev_position = numpy.array([odometry.pose.pose.position.x,odometry.pose.pose.position.y])
                self.prev_u_gtg = self.u_gtg
        elif action == 3:
            ...
            ratio = numpy.array([0.75,0.25,0,0])
        elif action == 5:
            ratio = numpy.array([0.5,0.5,0,0])
        elif action == 6:
            ratio = numpy.array([0.25,0.75,0,0])
    ...
    def _get_obs(self):
        ...
        observations = [round(x_position, 2),round(y_position, 2),round(base_yaw, 2),round(v, 2),round(theta_dot, 2),round(self.u_gtg[0,0], 2),round(self.u_gtg[1,0], 2)]+ discretized_laser_scan
        ...
        observations = [round(x_position, 2),round(y_position, 2),round(base_yaw, 2),round(v, 2),round(theta_dot, 2),round(self.joy_linear, 2),round(self.joy_angular, 2)]+discretized_laser_scan#[self.e_norm, round(self.delta_angle,2)]
        print(observations)
    ...
    def obstacle_avoidance(self,s):
        ...
        u_ao = 1/self.e_norm*(0.5/(self.e_norm**2+0.1))*e
        u_c = 2*numpy.array([[0,1],[-1,0]]).dot(u_ao)
        u_cc = 2*numpy.array([[0,-1],[1,0]]).dot(u_ao)
        self.a_ao = numpy.array([[1,0],[0,1/0.1]]).dot(numpy.array([[numpy.cos(-yaw),-numpy.sin(-yaw)],[numpy.sin(-yaw),numpy.cos(-yaw)]])).dot(u_ao)
        self.a_c = numpy.array([[1,0],[0,1/0.1]]).dot(numpy.array([[numpy.cos(-yaw),-numpy.sin(-yaw)],[numpy.sin(-yaw),numpy.cos(-yaw)]])).dot(u_c)
        self.a_cc = numpy.array([[1,0],[0,1/0.1]]).dot(numpy.array([[numpy.cos(-yaw),-numpy.sin(-yaw)],[numpy.sin(-yaw),numpy.cos(-yaw)]])).dot(u_cc)
        self.limitu()
        gdc = float(numpy.dot(self.u_gtg.T,u_c))
        gdcc = float(numpy.dot(self.u_gtg.T,u_cc))
        gdo = float(numpy.dot(self.u_gtg.T,u_ao))
        if len(self.prev_position) == 2:
            pdp = float(numpy.dot(numpy.array(s[0:2])-self.prev_position,self.prev_u_gtg))
        else:
            pdp = 10
        return [round(gdc,4),round(gdcc,4),round(gdo,4),round(pdp,4)]

    def limitu(self,):
        if abs(self.a_ao[0,0])>0.5 or abs(self.a_ao[1,0])>1:
            self.a_ao =self.a_ao/max(abs(self.a_ao/[[0.5],[1]]))
        if abs(self.a_c[0,0]>0.5) or abs(self.a_c[1,0])>1:
            self.a_c =self.a_c/max(abs(self.a_c/[[0.5],[1]]))
        if abs(self.a_cc[0,0])>0.5 or abs(self.a_cc[1,0])>1:
            self.a_cc =self.a_cc/max(abs(self.a_cc/[[0.5],[1]]))
    ...
    def _compute_reward(self, observations, done):
        ...
        if not done:
            ...
            if min_distance < 0.9 and min_distance>0.8:
                reward -= 10
    ...
    def respawnModel(self):
        if self.success:
            ...
            self.desired_point.x = random.choice([5,6,7,8])
            self.desired_point.y = random.choice([0,-4,-4.5,-2.5,-1.5,-0.5,0.5,4.5,-3.5,-3,-2,-1,5,1,-5])
            '''
