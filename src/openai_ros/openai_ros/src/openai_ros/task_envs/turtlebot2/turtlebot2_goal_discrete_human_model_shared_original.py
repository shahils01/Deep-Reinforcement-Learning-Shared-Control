...
class TurtleBot2DiscreteHumanModelEnv(turtlebot2_joy_env.TurtleBot2Env):
    def __init__(self):
        ...
        high = numpy.hstack((numpy.array([12,6,3.14,1,3.14,0.5,1]),6*numpy.ones([self.new_ranges])))#,numpy.array([12,6,3.14,1,3.14,0.5,1]),6*numpy.ones([self.new_ranges]),numpy.array([12,6,3.14,1,3.14,0.5,1]),6*numpy.ones([self.new_ranges])))
        ...
        low = numpy.hstack((numpy.array([-1,-1*6,-1*3.14,-1,-3.14,-0.5,-1]),numpy.zeros([self.new_ranges])))#,numpy.array([-1,-1*6,-1*3.14,-1,-3.14,-0.5,-1]),numpy.zeros([self.new_ranges]),numpy.array([-1,-1*6,-1*3.14,-1,-3.14,-0.5,-1]),numpy.zeros([self.new_ranges])))
    ...
    def _set_action(self, action):
        ...
        print("ratio:",+ratio)
        linear_speed = ratio[0]*self.observations[5]+ratio[1]*self.observations[7]+ratio[2]*self.observations[9]+ratio[3]*self.observations[11]
        angular_speed = ratio[0]*self.observations[6]+ratio[1]*self.observations[8]+ratio[2]*self.observations[10]+ratio[3]*self.observations[12]
    ...
    def _get_obs(self):
        ...
        self.observations = [round(x_position, 2),round(y_position, 2),round(base_yaw, 2),round(v, 2),round(theta_dot, 2), round(joy_linear, 2),round(joy_angular, 2)]+u+discretized_laser_scan
    ...
    def obstacle_avoidance(self,s):
        ...
        if n<=17:
            delta_angle = ((n-17)*10-5)/180*numpy.pi
        else:
            delta_angle = ((n-18)*10+5)/180*numpy.pi
        orientation = yaw + delta_angle
        e_norm = lidar[n]
        e = numpy.array([[-e_norm*numpy.cos(orientation)],[-e_norm*numpy.sin(orientation)]])
        u_ao = 1/e_norm*(1/(e_norm**2+0.1))*e
        u_c = 2*numpy.array([[0,1],[-1,0]]).dot(u_ao)
        u_cc = 2*numpy.array([[0,-1],[1,0]]).dot(u_ao)
        a_ao = numpy.array([[1,0],[0,1/0.1]]).dot(numpy.array([[numpy.cos(-yaw),-numpy.sin(-yaw)],[numpy.sin(-yaw),numpy.cos(-yaw)]])).dot(u_ao)
        a_c = numpy.array([[1,0],[0,1/0.1]]).dot(numpy.array([[numpy.cos(-yaw),-numpy.sin(-yaw)],[numpy.sin(-yaw),numpy.cos(-yaw)]])).dot(u_c)
        a_cc = numpy.array([[1,0],[0,1/0.1]]).dot(numpy.array([[numpy.cos(-yaw),-numpy.sin(-yaw)],[numpy.sin(-yaw),numpy.cos(-yaw)]])).dot(u_cc)
        return list(numpy.clip([a_ao[0,0],a_ao[1,0],a_c[0,0],a_c[1,0],a_cc[0,0],a_cc[1,0]],[-0.5, -1,-0.5, -1,-0.5, -1],[0.5,1,0.5,1,0.5,1]))
    ...
    def _compute_reward(self, observations, done):
        ...
        current_position.y = observations[1]
        current_position.z = 0.0
        min_distance = min(observations[-self.new_ranges:])
        current_yaw = observations[2]

        ################    ##################################

        xdiff = self.desired_point.x - current_position.x
        ydiff = self.desired_point.x - current_position.y
    ...
    def respawnModel(self):
        if self.success:
            ...
            self.desired_point.y = 0#random.choice([0,-4])
            '''
