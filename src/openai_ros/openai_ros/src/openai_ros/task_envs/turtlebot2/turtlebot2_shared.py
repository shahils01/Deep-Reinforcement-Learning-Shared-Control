...
class TurtleBot2SharedEnv(turtlebot2_env.TurtleBot2Env):
    def __init__(self):
        """
        ...
        ROSLauncher(rospackage_name="turtlebot_gazebo",
                    launch_file_name="shared.launch",
                    ros_ws_abspath=ros_ws_abspath)

        # Load Params from the desired Yaml file
        LoadYamlFileParamsTest(rospackage_name="openai_ros",
                               rel_path_from_package_to_file="src/openai_ros/task_envs/turtlebot2/config",
                               yaml_file_name="turtlebot2_shared.yaml")

        # Here we will add any init functions prior to starting the MyRobotEnv Pair class name
        super(TurtleBot2SharedEnv, self).__init__(ros_ws_abspath)
    ...
    def _get_obs(self):
        ...
        w_orientation = odometry.pose.pose.orientation.w

        # We round to only two decimals to avoid very big Observation space
        odometry_array = [round(x_position, 1),round(y_position, 1),round(x_orientation, 2),round(y_orientation, 2),round(z_orientation, 2),round(w_orientation, 2)]
        print(odometry_array)
    ...
    def _is_done(self, observations):
        ...
        current_position.x = observations[-6]
        current_position.y = observations[-5]
